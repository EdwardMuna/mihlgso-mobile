...
class LeadershipBoardScreen extends ConsumerWidget {
  const LeadershipBoardScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final async = ref.watch(leadershipBoardProvider);
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async => ref.invalidate(leadershipBoardProvider),
        child: async.when(
          loading: () => const LoadingView(),
          error: (e, _) => ErrorRetryView(message: e.toString(), onRetry: () => ref.invalidate(leadershipBoardProvider)),
          data: (content) {
            return ListView(
              ...
              children: [
                ...
                if (content.members.isEmpty)
                  EmptyView(message: strings.noBoardMembersYet, icon: Icons.groups_outlined)
                else
